...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query(value="SELECT * FROM event INNER JOIN article ON event.id = article.board.id WHERE event.categoryId = ?1 GROUP BY event.id ORDER BY count(*) desc LIMIT 1", nativeQuery = true)
    Event findEventTop1ByArticleCount(Integer categoryNo);
